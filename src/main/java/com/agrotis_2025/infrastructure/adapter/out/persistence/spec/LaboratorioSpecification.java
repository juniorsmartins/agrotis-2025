...
public final class LaboratorioSpecification {
    ...
    public static Specification<LaboratorioEntity> consultaDinamica(LaboratorioFiltroDto filtro) {

        return ((root, query, criteriaBuilder) -> {

            var predicados = new ArrayList<Predicate>();

            if (ObjectUtils.isNotEmpty(filtro.laboratorioId())) {
                var ids = Arrays.asList(filtro.laboratorioId().split(","));
                List<Predicate> idPredicates = ids.stream()
                        .filter(id -> !id.trim().isEmpty())
                        .map(id -> {
                            try {
                                UUID uuid = UUID.fromString(id.trim());
                                return criteriaBuilder.equal(root.get("laboratorioId"), uuid);
